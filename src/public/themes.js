...
const THEMES = {
  // ── Sombres ──────────────────────────────────────────────────────────────
  default:   { label:'Défaut',     emoji:'🌑', dark:true,  vars:{ '--bg':'#06060e','--surface':'#0d0d1c','--surface2':'#13132a','--border':'rgba(255,255,255,0.06)','--text':'#eeeef5','--muted':'rgba(238,238,245,0.38)','--accent':'#4c6ef5','--grid':'#1a1a3a','--cell':'#0d0d22','--red':'#ff2d55','--red-dim':'rgba(255,45,85,0.15)','--red-glow':'rgba(255,45,85,0.4)','--yellow':'#ffd60a','--yel-dim':'rgba(255,214,10,0.1)','--yel-glow':'rgba(255,214,10,0.4)','--green':'#30d158','--orange':'#ff9f0a' } },
  neon:      { label:'Néon',       emoji:'⚡', dark:true,  vars:{ '--bg':'linear-gradient(135deg,#050510,#0a0020,#050510)','--surface':'#0a0a20','--surface2':'#10102e','--border':'rgba(170,0,255,0.15)','--text':'#f0f0ff','--muted':'rgba(240,240,255,0.38)','--accent':'#aa00ff','--grid':'#160030','--cell':'#0a001a','--red':'#ff00aa','--red-dim':'rgba(255,0,170,0.15)','--red-glow':'rgba(255,0,170,0.6)','--yellow':'#00ffaa','--yel-dim':'rgba(0,255,170,0.1)','--yel-glow':'rgba(0,255,170,0.6)','--green':'#00ffaa','--orange':'#ff6600' } },
  fire:      { label:'Feu',        emoji:'🔥', dark:true,  vars:{ '--bg':'linear-gradient(to bottom,#0f0800,#1a0c00,#0f0500)','--surface':'#1a1000','--surface2':'#251800','--border':'rgba(255,102,0,0.12)','--text':'#fff8f0','--muted':'rgba(255,248,240,0.38)','--accent':'#ff3300','--grid':'#2a1000','--cell':'#1a0800','--red':'#ff6600','--red-dim':'rgba(255,102,0,0.15)','--red-glow':'rgba(255,102,0,0.6)','--yellow':'#ffdd00','--yel-dim':'rgba(255,221,0,0.1)','--yel-glow':'rgba(255,221,0,0.6)','--green':'#ffaa00','--orange':'#ff4400' } },
  ocean:     { label:'Océan',      emoji:'🌊', dark:true,  vars:{ '--bg':'linear-gradient(to bottom,#000d1a,#001428,#000a12)','--surface':'#001428','--surface2':'#001e3c','--border':'rgba(0,102,255,0.15)','--text':'#e8f4ff','--muted':'rgba(232,244,255,0.38)','--accent':'#0044cc','--grid':'#002040','--cell':'#001228','--red':'#0066ff','--red-dim':'rgba(0,102,255,0.15)','--red-glow':'rgba(0,102,255,0.6)','--yellow':'#00ffee','--yel-dim':'rgba(0,255,238,0.1)','--yel-glow':'rgba(0,255,238,0.6)','--green':'#00ffee','--orange':'#0099ff' } },
  forest:    { label:'Forêt',      emoji:'🌿', dark:true,  vars:{ '--bg':'linear-gradient(to bottom,#040d06,#091a0b,#020a03)','--surface':'#0a1a0d','--surface2':'#122518','--border':'rgba(0,200,80,0.1)','--text':'#e8f5ec','--muted':'rgba(232,245,236,0.38)','--accent':'#00aa44','--grid':'#0a2010','--cell':'#051008','--red':'#00cc55','--red-dim':'rgba(0,204,85,0.15)','--red-glow':'rgba(0,204,85,0.5)','--yellow':'#aaee00','--yel-dim':'rgba(170,238,0,0.1)','--yel-glow':'rgba(170,238,0,0.5)','--green':'#00ff88','--orange':'#88cc00' } },
  midnight:  { label:'Minuit',     emoji:'🌙', dark:true,  vars:{ '--bg':'linear-gradient(135deg,#0a0a12,#12101e,#08080f)','--surface':'#12121e','--surface2':'#1a1a2e','--border':'rgba(150,130,255,0.1)','--text':'#e8e8ff','--muted':'rgba(232,232,255,0.35)','--accent':'#6655ee','--grid':'#1e1830','--cell':'#120e20','--red':'#8866ff','--red-dim':'rgba(136,102,255,0.15)','--red-glow':'rgba(136,102,255,0.5)','--yellow':'#ffaa44','--yel-dim':'rgba(255,170,68,0.1)','--yel-glow':'rgba(255,170,68,0.5)','--green':'#44ddaa','--orange':'#ee7744' } },
  aurora:    { label:'Aurora',     emoji:'🌌', dark:true,  vars:{ '--bg':'linear-gradient(160deg,#020818,#051a14,#0a0520,#020c1a)','--surface':'#0a1020','--surface2':'#101828','--border':'rgba(0,220,150,0.1)','--text':'#e0fff8','--muted':'rgba(224,255,248,0.38)','--accent':'#00ddaa','--grid':'#0a1828','--cell':'#050c18','--red':'#00ccaa','--red-dim':'rgba(0,204,170,0.15)','--red-glow':'rgba(0,204,170,0.5)','--yellow':'#44ffcc','--yel-dim':'rgba(68,255,204,0.1)','--yel-glow':'rgba(68,255,204,0.5)','--green':'#00ffcc','--orange':'#44bbff' } },
  volcano:   { label:'Volcan',     emoji:'🌋', dark:true,  vars:{ '--bg':'linear-gradient(to bottom,#120000,#1a0400,#0a0000)','--surface':'#1a0800','--surface2':'#220e00','--border':'rgba(200,50,0,0.15)','--text':'#ffeee0','--muted':'rgba(255,238,224,0.38)','--accent':'#cc3300','--grid':'#2a0800','--cell':'#180400','--red':'#ff4400','--red-dim':'rgba(255,68,0,0.15)','--red-glow':'rgba(255,68,0,0.6)','--yellow':'#ffaa00','--yel-dim':'rgba(255,170,0,0.1)','--yel-glow':'rgba(255,170,0,0.6)','--green':'#ff6600','--orange':'#ff2200' } },
  galaxy:    { label:'Galaxie',    emoji:'🪐', dark:true,  vars:{ '--bg':'linear-gradient(135deg,#050012,#0a0025,#020010,#080018)','--surface':'#0c0820','--surface2':'#140e2e','--border':'rgba(120,80,255,0.12)','--text':'#ede8ff','--muted':'rgba(237,232,255,0.35)','--accent':'#7755ff','--grid':'#140a2a','--cell':'#0a0618','--red':'#aa66ff','--red-dim':'rgba(170,102,255,0.15)','--red-glow':'rgba(170,102,255,0.5)','--yellow':'#ffcc44','--yel-dim':'rgba(255,204,68,0.1)','--yel-glow':'rgba(255,204,68,0.5)','--green':'#44eebb','--orange':'#ff8844' } },
  blood:     { label:'Blood',      emoji:'🩸', dark:true,  vars:{ '--bg':'linear-gradient(to bottom,#0d0000,#1a0000,#080000)','--surface':'#180000','--surface2':'#220000','--border':'rgba(180,0,0,0.15)','--text':'#ffe8e8','--muted':'rgba(255,232,232,0.38)','--accent':'#cc0000','--grid':'#2a0000','--cell':'#180000','--red':'#ff1111','--red-dim':'rgba(255,17,17,0.15)','--red-glow':'rgba(255,17,17,0.6)','--yellow':'#ff6600','--yel-dim':'rgba(255,102,0,0.1)','--yel-glow':'rgba(255,102,0,0.6)','--green':'#aa0000','--orange':'#cc2200' } },
  // ── Clairs ───────────────────────────────────────────────────────────────
